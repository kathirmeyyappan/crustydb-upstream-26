...
mod test {
    ...
    fn test_tuple_gen() {
        let mut rng = get_rng();
        let t = gen_test_tuples(&mut rng, 10);
        assert_eq!(10, t.len());
    }
    ...
    fn test_uniform_strings() {
        let mut card = 10;
        let mut rng = get_rng();
        let mut strs = gen_uniform_strings(&mut rng, 100, Some(card), 10, 20);
        ...
        card = 100;
        map.clear();
        strs = gen_uniform_strings(&mut rng, 4800, Some(card), 10, 20);
        for i in strs {
            if let Field::String(val) = i {
                *map.entry(val).or_insert(0) += 1;
            }
    ...
    fn test_uniform_range() {
        let n = 1000;
        let mut rng = get_rng();
        let ints = gen_uniform_ints(&mut rng, n, Some(10));
        for x in &ints {
            if let Field::BigInt(a) = x {
                assert!(*a < 10 && *a >= 0);
            }
        }
    }
    ...
    fn test_uniform_ints() {
        let mut rng = get_rng();
        let mut ints = gen_uniform_ints(&mut rng, 4, Some(6));
        for x in &ints {
            if let Field::BigInt(a) = x {
                assert!(*a < 7);
            }
        ...
        ints = gen_uniform_ints(&mut rng, 1000, Some(card as u64));
        assert_eq!(1000, ints.len());

        let mut map = HashMap::new();
        for i in ints {
            if let Field::BigInt(val) = i {
                *map.entry(val).or_insert(0) += 1;
            }
        }
        ...
        card = 121;
        map.clear();
        ints = gen_uniform_ints(&mut rng, 10000, Some(card as u64));
        ...
        card = 500;
        map.clear();
        ints = gen_uniform_ints(&mut rng, card as u64, Some(card as u64));
        let mut map = HashMap::new();
        for i in ints {
            if let Field::BigInt(val) = i {
                *map.entry(val).or_insert(0) += 1;
