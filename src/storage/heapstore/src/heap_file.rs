...
pub(crate) struct HeapFile<T: MemPool> {
    ...
    current_data_page_id: AtomicPageId,
}
...
impl<T: MemPool> HeapFile<T> {
    ...
    pub fn new(c_id: ContainerId, mem_pool: Arc<T>) -> Result<Self, CrustyError> {
        ...
        let mut header_page = mem_pool.create_new_page_for_write(c_id).unwrap();
        let header_page_id = header_page.get_page_id();
        assert_eq!(header_page_id, 0);

        // Initialize the data page
        let mut data_page = mem_pool.create_new_page_for_write(c_id).unwrap();
        data_page.init_heap_page();
        let data_page_id = data_page.get_page_id();
        assert_eq!(header_page_id + 1, data_page_id);

        // Write the data page id to the header page
        header_page[0..std::mem::size_of::<PageId>()].copy_from_slice(&data_page_id.to_le_bytes());

        let heap_file = HeapFile {
            c_id,
            bp: mem_pool.clone(),
            header_page_id,
            current_data_page_id: AtomicPageId::new(data_page_id),
        };
        Ok(heap_file)
    }
    ...
    pub fn load(c_id: ContainerId, mem_pool: Arc<T>) -> Result<Self, CrustyError> {
        ...
        // Read the first page of the heapfile to get the next_page_id
        let header_page = mem_pool
            .get_page_for_read(PageFrameId::new(c_id, 0))
            .unwrap();
        let current_data_page_id = u32::from_le_bytes(
            header_page[0..std::mem::size_of::<PageId>()]
                .try_into()
                .unwrap(),
        );
        ...
        let heap_file = HeapFile {
            ...
            bp: mem_pool.clone(),
            header_page_id: 0,
            current_data_page_id: AtomicPageId::new(current_data_page_id),
        };
        Ok(heap_file)
    }
...
impl<T: MemPool> HeapFileIter<T> {
    ...
    fn initialize(&mut self) {
        ...
        let page_id = self.first_page;
        self.current_page = Some(self.get_page(page_id));
